...
const detailItem = { hidden: { opacity: 0, y: 8 }, visible: { opacity: 1, y: 0, transition: { duration: 0.25 } }, }
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl mx-auto px-16">
        ...
        <Tab.Group>
          {/* Tab headers */}
          <Tab.List ref={listRef} className="flex gap-2 border-b border-zinc-700 mb-6 relative">
            {jobs.map((job, index) => (
              <Tab
                key={job.title}
                as={motion.button}
